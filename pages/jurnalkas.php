<?php
  ...
  if($idt <> ''){
    //neraca
    $sql = mysqli_query($koneksi, "SELECT * FROM jurnal_kas WHERE id = '$idt'") or die(mysqli_error());
    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Jurnal Penerimaan Kas</h4>
                        <a href="index.php?menu=form-jurnalkas" class="btn btn-primary">Tambah <i class="typcn typcn-document btn-icon-prepend"></i></a>
                        <br><br>
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                $sql = mysqli_query($koneksi, "SELECT a.*, b.nama AS pelanggan FROM jurnal_kas AS a LEFT JOIN pelanggan AS b ON a.pelanggan = b.id ORDER BY tgl_penerimaan DESC LIMIT $limitStart, $limit") or die(mysqli_error());
                                $no = $limitStart + 1;
                                ...
                                while ($data = mysqli_fetch_array($sql)){
                                    ...
                                    echo"
                                        <tr>
                                            <td>".tgl_indo($data['tgl_penerimaan'])."</td>
                                            <td>$data[pelanggan]</td>
                                            <td>$data[no_faktur]</td>
                                            <td>$saldo</td>
                                            <td>
                                            <div class='d-flex align-items-center'>
                                                <a href='index.php?menu=form-jurnalkas&id=$data[id]' class='btn btn-success btn-sm btn-icon-text mr-3'>
                                                Edit
                                                <i class='typcn typcn-edit btn-icon-append'></i>                          
                                                </a>
                                                <a href=\"?menu=jurnalkas&idt=".$data["id"]."\" onClick=\"if (confirm('Yakin akan Hapus ".$data["nama_akun"]."?') == true) return true; else return false;\" class='btn btn-danger btn-sm btn-icon-text'>
                                                Hapus
                            ...
                            <nav>
                                <ul class="pagination justify-content-center">
                                    ...
                                    ?>
                                    <?php
                                    $SqlQuery = mysqli_query($koneksi, "SELECT a.*, b.nama AS pelanggan FROM jurnal_kas AS a LEFT JOIN pelanggan AS b ON a.pelanggan = b.id");        
